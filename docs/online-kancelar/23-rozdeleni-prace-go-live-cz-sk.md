...
| **10a** | Doplnění funkce odstoupení po auditu (20 §10.5): tlačítko od vytvoření objednávky (nezaplacená = storno), účet/IBAN jen u převodu (`requires_bank_account_for_refund`, `return_requests.refund_account`), informační bloky per trh (`market_settings.return_notice_md`, poplatek za vratku), znění potvrzovací obrazovky cs/sk, anonymní notifikace ambasadorovi | 20 §10.5 | pořadí: 12 → 10a → 11 |
| **11** | **Bankovní převod s VS** (R25): volba v pokladně, instrukce + QR (SPD CZ / PAY by square SK) na „Děkujeme“, v e-mailu a v Moje objednávka; `bank_payments_inbox` (ruční zápis + CSV import s mapováním), párování podle VS **a shody `payer_account`** (08 §8.5 b.2 — VS sám nestačí k automatickému sečtení víc plateb), tolerance nedoplatku, přeplatek do kreditu/vrácení, expirace 7 dní, potvrzení přes `fn_apply_payment_event('bank:{inbox_id}')`; platba po stornu zakládá **novou objednávku** s odkazem na tu původní, ne obnovu (08 §8.5 b.3); hlášení `PAYMENT-UNDERPAID/OVERPAID/UNMATCHED/LATE-AFTER-CANCEL`; admin „Převody“; pgTAP 079 vč. regresního testu na unikátnost VS (08 §8.5 b.5) | **08 §8 + §8.5 (R25)**, 21 C.3 | po bodu 10; účty z `legal_entities`, zapíná `bank_transfer_enabled` po IČO; §8.5 doplněno 23. 8. po interním auditu rizik VS |
| 12 | `payout_requests.returned` podle pravidla z 23. 8. (výplata vrácená bankou: ledger beze změny, `fn_admin_mark_payout_returned`, `fn_admin_resend_payout`, `fn_admin_cancel_returned_payout` = jediný návrat kreditu, blok nové žádosti, hlášení `PAYOUT-RETURNED`, pgTAP 078) | 21 C.5 | odsouhlaseno zadavatelem |
