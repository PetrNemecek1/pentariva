...
| **11** | **Bankovní převod s VS** (R25): volba v pokladně, instrukce + QR (SPD CZ / PAY by square SK) na „Děkujeme“, v e-mailu a v Moje objednávka; `bank_payments_inbox` (ruční zápis + CSV import s mapováním), párování podle VS, tolerance nedoplatku, přeplatek do kreditu/vrácení, expirace 7 dní, potvrzení přes `fn_apply_payment_event('bank:{inbox_id}')`; hlášení `PAYMENT-UNDERPAID/OVERPAID/UNMATCHED/LATE-AFTER-CANCEL`; admin „Převody“; pgTAP 079 | **08 §8 (R25)**, 21 C.3 | po bodu 10; účty z `legal_entities`, zapíná `bank_transfer_enabled` po IČO |
