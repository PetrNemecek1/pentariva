...
| R20 | **Multishop po zemích (22. 8. 2026):** jedna platforma, **trh = země** s vlastní měnou, cenami, DPH, akcemi, dopravou, právními texty a fakturující entitou. **Produkt se listuje jednou** (master) a per trh se aktivuje a doplňuje (cena, DPH, překlady v jazycích trhu, lokální média, akce); dokud není pro trh kompletní, je tam prostě neaktivní — **trhy se navzájem nikdy neblokují**. Účet a síť partnerů jsou globální, peníze se vedou per měna bez kurzových přepočtů v ledgeru. Strukturální část (trhy, měna, produkt per trh) se dělá **před go-live CZ**, funkčně CZ nic nemění. Závazné zadání: `21-revize-multishop-vyjimky.md` část B. |
...
| R25 | **Bankovní převod s VS (23. 8. 2026):** e-shop nabízí platbu převodem vedle karty — CZ v CZK na účet entity, SK v EUR na IBAN; VS = číslo objednávky, QR (SPD / PAY by square); ruční párování z výpisu v adminu (`bank_payments_inbox`), tolerance nedoplatku, přeplatek do kreditu nebo vrácení; nezaplaceno do 7 dnů = storno. Zapíná se přepínačem `bank_transfer_enabled` až po IČO a účtu. Specifikace `08` §8 (rozšířená) + `21` C.3. |
